...
def master_function(settings_file,output_file_format, output_format, processing_option, scenario_option, debugging_output, data_base_region, interpolate_missing_values):
    ...
    if processing_option != 'timeseries_only':
        ...
        for path in regular_parameter_paths:
            df_pivot, worksheet_name, data_overwritten = process_regular_parameters(path, unique_values_concatenated, rounding_df, output_format, scenario_option,debugging_output, data_base_region, interpolate_missing_values)
